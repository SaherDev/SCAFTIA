...
public class OutThread extends Thread {
    ...
    public OutThread(String mode,List<Neighbor> neighbors,String header,String msg ){
        this.mode=mode;
    ...
    public OutThread(String mode ,Neighbor neighbor,String headerOrSessionKey,String msgOrFile ){
        this.mode=mode;
        appSetting= new AppSetting();
        this.neighbors=new LinkedList<>();
                        ...
                        try {
                            ...
                            if (headerresponse.equals(Message.MESSAGE_OK)){
                               ...
                               //sending message to server and get responce back
                                ...
                                String msg=Message.createMessageToServer(appSetting.getUserName().trim(),n.getName().trim(),nonce);
                                ...
                                if (splitresponceFromServer.length==3){
                                    ...
                                    token=splitOuterMessageDec[3];
                                    String msgTmp=Helper.convertBytesToHex(Helper.stringToBytes(splitOuterMessageDec[0]))+" "+ inNonce+" "+receiver+" "+Helper.stringToBytes(token)+"";
                                    Log.addLine("RECEIVE",server.getIp() + ":" + server.getPort()," server",Message.MESSAGE_ACK_SESSION_KEY_REQUEST,msgTmp,ivGcm,"","Valid");
                                        ...
                                        if (firstChallengeMessageResponce.length==4){
                                            ...
                                            if (headerDec.trim().equals(Message.MESSAGE_CHALLENGE_ACK_SESSION)) {
                                                ...
                                                if (!nonceReceiverDec.equals(Message.MESSAGE_NO)){
                                                    ...
                                                    long noncechalenge = Long.parseLong(nonceReceiverDec) - 1;
                                                    iv = Cryptography.generate16IV();
                                                    secondChallengeMessage = Message.createChallengeMessage(sessionKey, iv, Message.MESSAGE_CHALLENGE_NONCE, appSetting.getUserName().trim(), noncechalenge + "");
